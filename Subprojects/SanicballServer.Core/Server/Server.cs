...
namespace SanicballCore.Server
{
    ...
    public class Server : IDisposable
    {
        ...
        private ConcurrentDictionary<WebSocket, WebSocketWrapper> connectedClients
            = new ConcurrentDictionary<WebSocket, WebSocketWrapper>();
        ...
        private async Task ListenerTask()
        {
            while (true)
            {
                var socket = await listener.AcceptWebSocketAsync(CancellationToken.None);
                var wrapper = new WebSocketWrapper(socket);
                wrapper.Start();

                Log($"Sending ping to {socket}");

                await socket.SendPingAsync();

                Log($"Recieved a socket connection from {socket}");

                connectedClients[socket] = wrapper;
            }
        }
        ...
        private void MessageLoop()
        {
            while (running)
            {
                ...
                foreach (var client in connectedClients)
                {
                    ...
                    {
                        using (var stream = new MemoryStream(msg.Data.ToArray(), false))
                        ...
                        using (var writer = new BinaryWriter(dest, Encoding.UTF8))
                        {
                            switch (msg.Type)
                            {
                                case MessageTypes.Discover:
                                    ...
                                    client.Value.Send(MessageTypes.Discover, writer, dest);

                                    Log("Sent discovery response to " + client.Key.RemoteEndpoint, LogType.Debug);

                                    break;
                                case MessageTypes.Validate:
                                    ClientInfo clientInfo = null;
                                    try
                                    {
                                        clientInfo = JsonConvert.DeserializeObject<ClientInfo>(reader.ReadString());
                                    }
                                    catch (JsonException ex)
                                    {
                                        ...
                                        Log($"Refused to validate {client.Key.RemoteEndpoint}");
                                        break;
                                    }
                                    ...
                                    Log($"Validated {client.Key.RemoteEndpoint}");

                                    break;
                                case MessageTypes.Connect:

                                    Log($"{client.Key.RemoteEndpoint} is now connected!");
                                ...
                                case MessageTypes.Disconnect:
                                    string statusMsg = reader.ReadString();
                                    ServClient associatedClient = clients.FirstOrDefault(a => a.Connection == msg.Source);
                                ...
                                case MessageTypes.PlayerMovement:
                                    writer.Write(msg.Data.ToArray());
                                    var recipients = clients.Where(a => a.Connection != msg.Source).ToList();
                                    if (recipients.Count > 0)
                                    {
                                        foreach (var item in recipients)
                                        {
                                            item.Connection.Send(MessageTypes.PlayerMovement, writer, dest);
                                        }
                                    }

                                    break;
                                case MessageTypes.Match:
                                    double timestamp = reader.ReadInt64();
                                    MatchMessage matchMessage = null;
                                    try
                                    {
                                        matchMessage = JsonConvert.DeserializeObject<MatchMessage>(reader.ReadString(), new JsonSerializerSettings() { TypeNameHandling = TypeNameHandling.All });
                                    }
                                    catch (JsonException ex)
                                    {
                                    ...
                                    {
                                        CheckpointPassed(matchMessage);
                                    }

                                    if (matchMessage is DoneRacingMessage)
                                    {
                                        DoneRacing(matchMessage);
                                    }
        ...
        private void ClientJoined(QueuedMessage msg, MatchMessage matchMessage)
        {
            var castedMsg = (ClientJoinedMessage)matchMessage;

            ServClient newClient = new ServClient(castedMsg.ClientGuid, castedMsg.ClientName, msg.Source);
        ...
        private void PlayerJoined(QueuedMessage msg, MatchMessage matchMessage)
        {
            var castedMsg = (PlayerJoinedMessage)matchMessage;

            //Check if the message was sent from the same client it wants to act for
            ServClient client = clients.FirstOrDefault(a => a.Connection == msg.Source);
        ...
        private void PlayerLeft(QueuedMessage msg, MatchMessage matchMessage)
        {
        ...
        private void CharacterChanged(QueuedMessage msg, MatchMessage matchMessage)
        {
        ...
        private void ChangedReady(QueuedMessage msg, MatchMessage matchMessage)
        {
        ...
        private void StartRace(QueuedMessage msg)
        {
        ...
        private void ChatMessage(QueuedMessage msg, MatchMessage matchMessage)
        {
        ...
        private void LoadLobby(QueuedMessage msg)
        {
            ServClient client = clients.FirstOrDefault(a => a.Connection == msg.Source);
        ...
        private void CheckpointPassed(MatchMessage matchMessage)
        {
        ...
        private void DoneRacing(MatchMessage matchMessage)
        {
        ...
        {
            Log("Sending message of type " + matchMsg.GetType() + " to " + clients.Count + " connection(s)", LogType.Debug);
            ...
            using (var writer = new BinaryWriter(memory, Encoding.UTF8, false))
            {
                ...
                {
                    item.Connection.Send(MessageTypes.Match, data);
                }
            }
        }
        ...
        private void SendTo(MatchMessage matchMsg, ServClient reciever)
        {
            ...
            using (var writer = new BinaryWriter(memory, Encoding.UTF8, false))
            {
                ...
                var data = memory.ToArray();
                reciever.Connection.Send(MessageTypes.Match, data);
            }
        }
